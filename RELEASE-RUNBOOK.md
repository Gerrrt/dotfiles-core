...
| **htpx** | `htpx` (`CHANGELOG.md`) | push a CHANGELOG bump to `main` | `dotfiles-Kali` (`companion.lock`) | [3](#3-cut-an-htpx-release) |

Core and htpx are independent: a Core release bumps each OS repo's `core.lock`; an htpx
release bumps Kali's `companion.lock`. They update different files and never collide.
...
1. `release.yml` publishes the GitHub Release (notes via git-cliff; preview with `make release-notes`).
...
prep still fails loud. `dotfiles-Windows` is not in the subtree fan-out; it mirrors
`nvim/` + `starship.toml` via its own sync scripts.

---

## 3. Cut an htpx release
...
## 4. Before relying on a new cross-repo workflow
...
| `sync-fanout` skips with `'' is not a clean vX.Y.Z release tag` | triggered from a commit with no tag on it | use the manual backfill (section 3) with `tag: vX.Y.Z` |
